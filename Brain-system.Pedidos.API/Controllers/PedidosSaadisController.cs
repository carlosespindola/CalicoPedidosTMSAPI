...
namespace BrainSystem.Pedidos.API.Controllers
{
    ...
    public class PedidosSaadisController : ControllerBase
    {
        private SaadisDBContextFactory _saadisFactory;
        readonly ISaadisDBContextFactory _dapperSaadisDBContextFactory;
        private string _cliente;

        public PedidosSaadisController(IHttpContextAccessor contextAccessor, SaadisDBContextFactory saadisFactory, ISaadisDBContextFactory dapperSaadisDBContextFactory)
        {
            ClaimsPrincipal currentUser = contextAccessor.HttpContext.User;

            _cliente = currentUser.FindFirstValue("id");

            _saadisFactory = saadisFactory;

            _dapperSaadisDBContextFactory = dapperSaadisDBContextFactory;
        }
        ...
        [Produces("application/json")]
        [HttpPost("Alta")]
        public async Task<IActionResult> Post([FromBody] PedidoSaadis[] pedidos)
        {
            ...
            try
            {
                List<ResultadoPedidoSaadis> resultadoPedidos = await AltaPedidosSaadis(_cliente, pedidos, _saadisDBContext, _dapperSaadisDBContext);
        ...
        public async Task<IActionResult> Delete([FromBody] IdentificadorPedidoSaadis[] pedidos)
        {
            ...
            try
            {
                ...
                foreach (IdentificadorPedidoSaadis item in pedidos)
                {
                    ...
                    ResultadoPedidoSaadis resultado = new ResultadoPedidoSaadis(item.PuntoDeVentaComprobante, item.NumeroComprobante, item.LetraComprobante);
